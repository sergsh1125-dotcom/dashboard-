...
def style_function(feature):
    ...
    coverage = coverage_dict.get(region,0)
    # затемнення для інших регіонів при фільтрі
    fill_opacity = 0.75
    if selected_region != "Всі" and region != region_name_map.get(selected_region,""):
        fill_opacity = 0.2
    return {"fillColor": color_by_coverage(coverage),"color":"black","weight":1,"fillOpacity":fill_opacity}
...
tooltip = GeoJsonTooltip(
    ...
    sticky=True,
    localize=True,
    fmt=lambda x: f"{x} - {coverage_dict.get(x,0)}%"
)
